...
charge.drop_duplicates(inplace = True, ignore_index = True)
# charge.drop(["ID", "CHARGE_DATE", "DISCHARGE_DATE"], axis = 1, inplace = True)
patron.drop_duplicates(["ID"], inplace = True, ignore_index = True)
item.drop_duplicates(["TITLE"], inplace = True, ignore_index = True)
item_ = set(item["ID"])
not_in_index = charge.index[~charge['ITEM_ID'].isin(item_)]
charge.drop(not_in_index, axis = 0, inplace = True)
patron_ = set(patron["ID"])
not_in_index = charge.index[~charge['PATRON_ID'].isin(patron_)]
charge.drop(not_in_index, axis = 0, inplace = True)
...
def refinement(patron, item, charge):
    ...
    item.index = item["ID"]
    # patron.drop(["new_ID"], axis = 1, inplace = True)
    ...
    patron = patron.sort_values(["MAJOR", "GENDER", "BIRTHDAY"])
    # item = item.sort_values(["CLASS_NO", "target"])
    patron.index = [i for i in range(len(patron))]
    # item.index = [i for i in range(len(item))]
...
patron_dic = {}
swap_patron_dic = {}
for idx in patron.index:
    patron_dic[patron.loc[idx, "ID"]] = idx
    swap_patron_dic[idx] = patron.loc[idx, "ID"]

# item_dic = {}
# swap_item_dic = {}
# for idx in item.index:
#     item_dic[item.loc[idx, "ID"]] = idx
#     swap_item_dic[idx] = item.loc[idx, "ID"]

for i in tqdm(range(len(charge))):
    charge.loc[i, "PATRON_ID"] = patron_dic[charge.loc[i, "PATRON_ID"]]
    # if charge.loc[i, "ITEM_ID"] not in item_dic.keys():
    #     charge.loc[i, "ITEM_ID"] = np.NaN
    #     continue
    # charge.loc[i, "ITEM_ID"] = item_dic[charge.loc[i, "ITEM_ID"]]

charge = charge.sort_values(by = ["PATRON_ID"])
...
ind_train = indices[:int(len(indices)*0.8)]
ind_val = indices[int(len(indices)*0.8):]
